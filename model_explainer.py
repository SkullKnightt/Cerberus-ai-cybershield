...
# Specifically suppress sklearn warnings
...
class ModelExplainer:
    ...
    def explain_prediction(self, feature_vector, feature_names):
        ...
        try:
            ...
            # Silently use feature importances instead
            if hasattr(self.model, 'feature_importances_'):
                ...
                # If no feature importance is available, return minimal explanation
                return {
                    'prediction': int(prediction),
                    'confidence': float(confidence),
                    'top_features': [("Unknown", 0) for _ in range(10)],
                    'method': 'none'
                }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Malware detection model explainer')
    parser.add_argument('--explain', type=str, help='Path to file for explanation')
    ...
    if args.explain:
        if os.path.exists(args.explain):
            try:
                # Extract features from the file
                features = explainer.feature_extractor.extract_features(args.explain)
                feature_names = features.columns.tolist()
                
                # Get feature values as a list
                feature_values = features.iloc[0].values.tolist()
                
                # Explain the prediction
                explanation = explainer.explain_prediction(feature_values, feature_names)
                print("\nEXPLANATION RESULTS:")
                print("=====================")
                print(f"File: {args.explain}")
                print(f"Prediction: {'MALWARE' if explanation['prediction'] == 1 else 'BENIGN'}")
                print(f"Confidence: {explanation['confidence']*100:.2f}%")
                
                # Print explanation method used if in verbose mode
                if 'method' in explanation and explanation['method'] != 'shap':
                    print(f"(Using {explanation['method']} explanation method)")
                
                print("\nTop contributing features:")
                for feature, value in explanation['top_features']:
                    direction = "increases" if value > 0 else "decreases"
                    print(f"  • {feature}: {abs(value):.4f} ({direction} malware probability)")
                
                # Generate visual explanation
                output_path = f"{os.path.basename(args.explain)}_explanation.png"
                explainer.generate_explanation_plot(explanation, output_path)
        ...
        print("Model evaluation requires test data. Please provide X_test and y_test arguments.")
    else:
        parser.print_help() 